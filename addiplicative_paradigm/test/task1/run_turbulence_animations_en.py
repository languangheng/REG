"""
REG Turbulence Simulation - Batch Execution
Running 3 dynamic visualization simulations and saving results
"""
import numpy as np
...
print("=" * 70)
print("REG Turbulence Dynamic Simulation - Batch Execution")
print("=" * 70)
print("\nNote: Each simulation opens a dynamic window.")
print("Close the window to continue to the next simulation.\n")

# ============ Script 1: 1D Turbulence Transition ============
print("\n" + "=" * 70)
print("[1/3] 1D Turbulence Transition Simulation")
print("=" * 70)
...
ax1.set_title('Dependency Depth Profile', color='white'); ax1.axhline(1.0, color='gray', linestyle='--')
...
ax2.set_title('Spatiotemporal Evolution (Red=Collapse)', color='white')

ax3 = axes1[1, 0]; ax3.set_xlim(0, 500); ax3.set_ylim(0, 1.5)
ax3.set_title('Turbulence Intensity', color='white'); ax3.axhline(0.3, color='gray', linestyle='--')
...
def update1(frame):
    ...
    if turb < 0.2: regime, color = 'Laminar', 'cyan'
    elif turb < 0.5: regime, color = 'Transition', 'yellow'
    else: regime, color = 'Turbulent', 'red'
    if regime != last_regime1:
        msg = f"[Step {net1.step_count:>4d}] Flow={net1.flow_drive:.3f} sigma={turb:.4f} *** {last_regime1} -> {regime} ***"
        print(msg)
    ...
    status_str = f"Step: {net1.step_count}\nFlow: {net1.flow_drive:.3f}\nIntensity: {turb:.4f}\nState: {regime}"
    status_text1.set_text(status_str); status_text1.set_color(color)
    fig1.suptitle(f'REG 1D Turbulence | Step {net1.step_count} | Flow {net1.flow_drive:.2f} | {regime}', fontsize=16, fontweight='bold', color='white')
...
print("\n[COMPLETE] 1D turbulence simulation done. Saved to turbulence_1d_final.png")
print(f"State transitions: {len(transition_log1)}")

# ============ Script 2: 2D Vortex Emergence ============
print("\n" + "=" * 70)
print("[2/3] 2D Vortex Emergence Simulation")
print("=" * 70)
...
cbar1 = plt.colorbar(im1_2, ax=axes2[0,0]); cbar1.set_label('Depth', color='white')

im2_2 = axes2[0,1].imshow(np.sqrt(net2.vx**2+net2.vy**2), cmap='hot', vmin=0, vmax=0.1, origin='lower')
axes2[0,1].set_title('Vortex Intensity (Velocity Magnitude)', color='white')
cbar2 = plt.colorbar(im2_2, ax=axes2[0,1]); cbar2.set_label('Velocity', color='white')

im3_2 = axes2[1,0].imshow(net2.history, cmap='plasma', vmin=0, vmax=3, origin='lower')
axes2[1,0].set_title('Vortex Activity History (Bright=Frequent)', color='white')
cbar3 = plt.colorbar(im3_2, ax=axes2[1,0]); cbar3.set_label('Cumulative', color='white')
...
def update2(frame):
    ...
    if turb < 0.15: regime, color = 'Laminar', 'cyan'
    elif turb < 0.4: regime, color = 'Transition', 'yellow'
    else: regime, color = 'Turbulent', 'red'
    if regime != last_regime2:
        msg = f"[Step {net2.step_count:>4d}] Flow={net2.flow_drive:.3f} sigma={turb:.4f} *** {last_regime2} -> {regime} ***"
        print(msg)
    ...
    status_text2.set_text(f"Step: {net2.step_count}\nFlow: {net2.flow_drive:.3f}\nIntensity: {turb:.4f}\nState: {regime}")
    status_text2.set_color(color)
    fig2.suptitle(f'REG 2D Turbulence | Step {net2.step_count} | Flow {net2.flow_drive:.2f} | {regime}', fontsize=16, fontweight='bold', color='white')
...
# Save 2D simulation results
fig2.savefig('turbulence_2d_final.png', dpi=150, bbox_inches='tight')
print("\n[COMPLETE] 2D turbulence simulation done. Saved to turbulence_2d_final.png")
print(f"State transitions: {len(transition_log2)}")

# ============ Script 3: 3D Vortex Tube Structure ============
print("\n" + "=" * 70)
print("[3/3] 3D Vortex Tube Structure Simulation")
print("=" * 70)
...
cbar1_3 = plt.colorbar(im1_3, ax=axes3[0,0]); cbar1_3.set_label('Depth', color='white')
...
cbar2_3 = plt.colorbar(im2_3, ax=axes3[0,1]); cbar2_3.set_label('Velocity', color='white')

im3_3 = axes3[1,0].imshow(net3.history_slice, cmap='plasma', vmin=0, vmax=3, origin='lower')
axes3[1,0].set_title('Vortex Tube History (Bright=Frequent)', color='white')
cbar3_3 = plt.colorbar(im3_3, ax=axes3[1,0]); cbar3_3.set_label('Cumulative', color='white')
...
def update3(frame):
    ...
    if regime != last_regime3:
        msg = f"[Step {net3.step_count:>4d}] Flow={net3.flow_drive:.3f} sigma={turb:.4f} *** {last_regime3} -> {regime} ***"
        print(msg)
    ...
    status_text3.set_text(f"Step: {net3.step_count}\nFlow: {net3.flow_drive:.3f}\nIntensity: {turb:.4f}\nState: {regime}")
    status_text3.set_color(color)
    fig3.suptitle(f'REG 3D Turbulence | Step {net3.step_count} | Flow {net3.flow_drive:.2f} | {regime}', fontsize=16, fontweight='bold', color='white')
...
# Save 3D simulation results
fig3.savefig('turbulence_3d_final.png', dpi=150, bbox_inches='tight')
print("\n[COMPLETE] 3D turbulence simulation done. Saved to turbulence_3d_final.png")
print(f"State transitions: {len(transition_log3)}")
...
print(f"  1D Turbulence: {len(transition_log1)} state transitions")
print(f"  2D Turbulence: {len(transition_log2)} state transitions")
print(f"  3D Turbulence: {len(transition_log3)} state transitions")
print("\nAll dynamic simulations completed!")
