...
if not os.path.exists('turbulence_1d_results.png'):
    ...
    print(f"  [SKIP] 已存在 turbulence_1d_results.png")
...
if not os.path.exists('turbulence_scan_results.png'):
    ...
    print(f"  [SKIP] 已存在 turbulence_scan_results.png")
...
if not os.path.exists('turbulence_2d_results.png'):
    size = 60
    ...
    print(f"  [SKIP] 已存在 turbulence_2d_results.png")

# ============ 脚本4: 三维涡管结构模拟（优化版） ============
print("\n[4/5] 三维涡管结构模拟（优化版，减少计算量）...")

class FluidREG_3D:
    def __init__(self, size=20, dep_strength=1.0, flow_drive=0.03, seed=42):
        np.random.seed(seed)
        self.size = size
        self.dep_strength = dep_strength
        self.flow_drive = flow_drive
...
mid = SIZE // 2
fig, axes = plt.subplots(1, 3, figsize=(18, 5))
fig.suptitle(f'REG 三维湍流 | 涡管结构 (网格: {SIZE}x{SIZE}x{SIZE})', fontsize=16, fontweight='bold')
